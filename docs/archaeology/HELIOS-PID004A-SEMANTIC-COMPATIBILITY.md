...
**There is deliberately no global timeframe-to-semantic-role mapping anywhere in HELIOS.** Confirmed at the code level: `docs/COMPOSITION.md` §1.2 states "Nothing in `helios/composition/` names a timeframe... `tests/test_chain_multi_timeframe.py` asserts this mechanically: no `Timeframe` code or HERMES code appears as an identifier or string literal anywhere in the engine's source." The PID's GOLD template (`CONTEXT`→4H, `LOCATION`→1H, `CONFIRMATION`→15M, `TRIGGER`→5M) is *per-package configuration* declared in each package's `inputs` block (`helios/spec/model.py::InputRequirement`, one `(role, timeframe)` binding per input) — never a HELIOS constant.
