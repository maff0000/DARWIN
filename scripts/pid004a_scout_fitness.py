...
def attempt_specification(discovery: dict) -> SpecificationDraft:
    ...
    instrument_id = discovery.get("source_symbol") or discovery.get("sourceSymbol")
    applicability = (
        InstrumentApplicability(kind=InstrumentApplicabilityKind.EXPLICIT_SINGLE, instrument_ids=(str(instrument_id),))
        if instrument_id
        else InstrumentApplicability(
            kind=InstrumentApplicabilityKind.INSTRUMENT_GENERIC,
            generic_criteria=("source_symbol not recorded on this real discovery",),
        )
    )
